...
public class DayTimeTableDao {
    ...
    public DayTimeTable getDayTimeTableByDay(String day) {
        try {
            return entityManager.createQuery("SELECT dtt FROM DayTimeTable dtt WHERE dtt.day = :day", DayTimeTable.class)
    ...
    public List<DayTimeTable> getAllDayTimeTables() {
        return entityManager.createQuery("SELECT dtt FROM DayTimeTable dtt", DayTimeTable.class).getResultList();
    }
