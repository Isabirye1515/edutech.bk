...
public class TimeProgramDao {
  ...
  public void deleteTimeProgramByTitle(String title) {
    entityManager
        .createQuery("DELETE FROM TimeProgram tp WHERE tp.title = :title")
  ...
  public TimeProgram getTimeProgramByTitle(String title) {
    try {
      return entityManager
          .createQuery("SELECT tp FROM TimeProgram tp WHERE tp.title = :title", TimeProgram.class)
          .setParameter("title", title)
          .getSingleResult();
    } catch (Exception e) {
  ...
  public List<TimeProgram> getAllTimePrograms() {
    return entityManager
        .createQuery("SELECT tp FROM TimeProgram tp", TimeProgram.class)
        .getResultList();
  }
}
