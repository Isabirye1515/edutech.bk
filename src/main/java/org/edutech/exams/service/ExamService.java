...
public class ExamService {
    ...
    public List<ExamDTO> getAllExams() {
        return examDa.getAllExams().stream()
                .map(this::toDTO)
...
private ExamDTO toDTO(Exam exam) {
    ...
    // Map supervisors
    List<SuperVisorDTO> supervisorDTOs = exam.getSupervisors() != null
        ? exam.getSupervisors().stream().map(s -> {
    ...
    // Map exam students
    List<ExamStudentDTO> examStudentDTOs = exam.getExamStudents() != null
        ? exam.getExamStudents().stream().map(examS -> {
            ...
            List<PaperRequestDTO> paperDTOs = examS.getPapers() != null
                ? examS.getPapers().stream().map(paper -> {
                    ...
                    List<MarksDTO> marksDTOs = paper.getMarks() != null
                        ? paper.getMarks().stream().map(mark -> {
                            MarksDTO marksDTO = new MarksDTO();
                            marksDTO.setId(mark.getId());
                            marksDTO.setPaperName(mark.getPaperName());
                            marksDTO.setMarkedBy(mark.getMarkedBy());
                            marksDTO.setPaperCode(mark.getPaperCode());
